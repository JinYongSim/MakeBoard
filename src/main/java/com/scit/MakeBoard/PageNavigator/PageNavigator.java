package com.scit.MakeBoard.PageNavigator;
...
public class PageNavigator {
	...
	private int currentPerGroup; // 현재페이지 그룹
	//구해야하것
	...
	public PageNavigator(int boardPerPage, int pagePerGroup,int currentPage,int totalRecord) {
		...
		// 현재 속한 그룹(0부터 시작 가능)
		currentPerGroup = (this.currentPage - 1) / pagePerGroup;
		// 현재 속한 그룹의 첫페이지
		startPageGroup = currentPerGroup * pagePerGroup + 1;
		if(startBoardPage<1) {
			startPageGroup=1;
		}
	...
	public int getCurrentPerGroup() {
		return currentPerGroup;
	}
	public void setCurrentPerGroup(int currentPerGroup) {
		this.currentPerGroup = currentPerGroup;
	}
